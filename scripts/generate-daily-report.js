...
/**
 * OpenClaw 场景报告生成器
 * 1. 每日独立报告 - reports/YYYY-MM-DD.html
 * 2. 总览汇总页 - reports/index.html (包含所有场景)
 * 3. 归档索引页 - reports/archive.html (列出所有日报)
 */
  ...
  <div class="container">
    <div class="header">
      ...
      <div class="meta">
        ...
        <span class="meta-item">⏰ 更新时间：${new Date().toLocaleString('zh-CN')}</span>
      </div>
    </div>
...
// Load all historical data and generate overview
const allScenarios = [];
const dailyReports = [];
...
existingRawFiles.forEach(f => {
  ...
  scenarios.forEach(s => {
    s.collectDate = date; // Mark collection date
    allScenarios.push(s);
  });
...
// Generate overview page (index.html)
